...
// Rooms stored before the `type` field existed get one by known id.
const LEGACY_TYPE = { majlis: "living", kitchen: "kitchen", bedroom: "bedroom", bathroom: "bathroom" };
export const migrateRooms = (rooms) =>
  (rooms || []).map((r) => (r.type ? r : { ...r, type: LEGACY_TYPE[r.id] || "general" }));
...
// Builds today's list from rooms for the mode, minus skipped ids, plus
// mom's ad-hoc extras. Done state and manual order carry over from prev.
